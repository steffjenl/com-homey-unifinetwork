...
class WiredClient extends Homey.Device {
  ...
  async initWiredClient() {
    ...
    // Snapshot trigger
    this._snapshotTrigger = new Homey.FlowCardTrigger(Constants.EVENT_SNAPSHOT_CREATED);
    this._snapshotTrigger.register();

    // Connection Status trigger
    this._connectionStatusTrigger = new Homey.FlowCardTrigger(Constants.EVENT_CONNECTION_CHANGED);
    this._connectionStatusTrigger.register();

    // Connection Status trigger
    this._doorbellRingingTrigger = new Homey.FlowCardTrigger(Constants.EVENT_DOORBELL_RINGING);
    this._doorbellRingingTrigger.register();

    // Action 'take snapshot'
    new Homey.FlowCardAction(Constants.ACTION_TAKE_SNAPSHOT)
        .register()
    ...
    new Homey.FlowCardAction(Constants.ACTION_SET_RECORDING_MODE)
        .register()
        .registerRunListener((args, state) => {
          if (typeof args.device.getData().id !== 'undefined') {
